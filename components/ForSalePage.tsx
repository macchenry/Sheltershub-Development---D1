...
// Local Sale Card to support specific "Location" requirement
const SalePropertyCard: React.FC<{ property: Property; onClick: () => void }> = ({ property, onClick }) => {
  ...
  // Mock location based on ID since strictly typed Property interface doesn't have address
  const mockLocations = ["East Legon, Accra", "Cantonments, Accra", "Airport Residential", "Osu, Accra", "Tema, Comm 11", "Spintex Road"];
    ...
    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden group transition-shadow duration-300 shadow-sm hover:shadow-lg flex flex-col h-full">
      <div className="relative cursor-pointer" onClick={handleCardClick}>
        ...
        <div className="absolute top-4 left-4 flex items-center gap-2">
          ...
          {property.isPremium && (
             <span className="bg-[#F9A826] text-white text-xs font-bold px-3 py-1.5 rounded-md shadow uppercase tracking-wide">Featured</span>
          )}
        </div>
...
const ForSalePage: React.FC<ForSalePageProps> = ({ onNavigate }) => {
  ...
  useEffect(() => {
    window.scrollTo(0, 0);
    // Initial load: Get items for sale.
    const sales = allPropertiesList.filter(p => p.status === PropertyStatus.ForSale);
    // If mock data generation resulted in too few, fallback to showing some sold items or others for visual completeness
    setFilteredProperties(sales.length > 0 ? sales : allPropertiesList.slice(0, 9)); 
  }, []);
  ...
  const applyFilters = (e: React.FormEvent) => {
    e.preventDefault();
    let result = allPropertiesList.filter(p => p.status === PropertyStatus.ForSale);
  ...
  const handleReset = () => {
    ...
    setFilteredProperties(allPropertiesList.filter(p => p.status === PropertyStatus.ForSale));
  };

  return (
    <div className="bg-gray-50 min-h-screen font-sans">
      <Header onNavigate={onNavigate} activePage="for-sale" />

      {/* Page Title Section */}
      <div className="bg-[#0A2B4C] text-white py-16">
        <div className="container mx-auto px-4 text-center">
            <h1 className="text-3xl md:text-4xl font-bold mb-3">For Sale Properties</h1>
            <p className="text-blue-100 text-lg font-light max-w-2xl mx-auto">Browse verified properties available for purchase across prime locations.</p>
        </div>
      </div>
      ...
      <main className="container mx-auto px-4 py-8 -mt-8 relative z-10">
        ...
        {filteredProperties.length > 0 ? (
            ...
            <div className="text-center py-16 bg-white rounded-xl border border-gray-200 shadow-sm mt-8">
                ...
                <h3 className="text-xl font-bold text-[#0A2B4C] mb-2">No properties for sale found</h3>
